...
<xsl:stylesheet version="2.0" xmlns="http://rubicon.eu/1.1/DepXML" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ru="http://www.rubicon.eu" exclude-result-prefixes="ru">
	...
	<xsl:template match="/">
				...
				<xsl:for-each select="//Assemblies/Assembly">
					...
					<xsl:if test="count(//*[@assembly-ref = $assembly/@id]) > 0">
						<xsl:element name="Assembly">
							...
							<xsl:attribute name="culture"><xsl:value-of select="@culture"/></xsl:attribute>
        ...
        <xsl:for-each-group select="//InvolvedTypes/InvolvedType/Mixins/Mixin/TargetCallDependencies/Dependency[@is-interface = 'true']" group-by="string-join(@*, '?')">
          <xsl:element name="Dependency">
	...
	<xsl:template match="*" mode="cleanUpSignature">
		<xsl:choose>
			...
			<xsl:when test="name(.) = 'ExplicitInterfaceName'"><xsl:value-of select="concat(ru:GetNameWithoutGenericParameters(.), replace(., '[^,&gt;&lt;]', ''))" /></xsl:when>
			<xsl:when test="name(.) = 'Keyword' or name(.) = 'Name'"><xsl:value-of select="." /><xsl:text> </xsl:text></xsl:when>
			<xsl:when test="name(.) = 'Type'"><xsl:value-of select="if (./@genericParameter) then @genericParameter else ." /><xsl:text> </xsl:text></xsl:when>
	...
	<xsl:template name="dependencyPart">
		...
		<xsl:attribute name="metadataToken"><xsl:value-of select="$type/@metadataToken"/></xsl:attribute>
		<xsl:attribute name="assemblyRef"><xsl:value-of select="$type/@assembly-ref"/></xsl:attribute>
		<xsl:attribute name="assemblyName"><xsl:value-of select="$assemblies[@id = $type/@assembly-ref]/@name" /></xsl:attribute>
		<xsl:attribute name="typeType">
		<xsl:choose>
			<xsl:when test="$type/@is-interface = 'true'"><xsl:text>Interface</xsl:text></xsl:when>
			<xsl:otherwise><xsl:text>Class</xsl:text></xsl:otherwise>
		</xsl:choose></xsl:attribute>
		<xsl:attribute name="fullyQualifiedTypeName"><xsl:value-of select="concat($type/@namespace, '.', ru:GetNameWithoutGenericParameters($type/@name), ru:GetGenericTypeParameterCount($type/@name))"/></xsl:attribute>
	</xsl:template>
  ...
  <xsl:template match="Dependency" mode="referenced">
	...
	<xsl:template name="memberPart">
		<xsl:choose>
			<xsl:when test="@type = 'Method' or @type = 'Constructor'"><xsl:call-template name="methodElement" /></xsl:when>
			<xsl:when test="@type = 'Event'"><xsl:call-template name="eventElement" /></xsl:when>
			<xsl:when test="@type = 'Property'"><xsl:call-template name="propertyElement" /></xsl:when>
		</xsl:choose>
	</xsl:template>
