...
namespace Remotion.Validation.UnitTests.Validators
{
  ...
  public class NotEqualValidatorTest : ValidatorTestBase
  {
    ...
    [Test]
    public void Validate_WithValueTypeAndValueTypeEqualsComparisonValueAndCustomValidationMessage_ReturnsSingleValidationFailureWithCustomValidationMessage ()
    {
      var propertyValidatorContext = CreatePropertyValidatorContext (1);
      var validator = new NotEqualValidator (1, validationMessage: new InvariantValidationMessage ("Custom validation message: '{ComparisonValue}'."));
      ...
      Assert.That (validationFailures[0].ErrorMessage, Is.EqualTo ("Enter a value not equal to '1'."));
    ...
    public void Validate_WithValueTypeAndValueTypeEqualsComparisonValue_ReturnsSingleValidationFailure ()
    {
      ...
      Assert.That (validationFailures.Length, Is.EqualTo (1));
      Assert.That (validationFailures[0].ErrorMessage, Is.EqualTo ("Enter a value not equal to '1'."));
      Assert.That (validationFailures[0].LocalizedValidationMessage, Is.EqualTo ("Enter a value not equal to '1'."));
    }
    ...
    {
      var propertyValidatorContext = CreatePropertyValidatorContext (1);
      var validator = new NotEqualValidator (1f);
    ...
    public void Validate_WithReferenceTypeAndReferenceTypeNotEqualsComparisonReference_ReturnsNoValidationFailures ()
    {
      ...
      var validator = new NotEqualValidator (new object());
    ...
    public void Validate_WithReferenceTypeAndReferenceTypeEqualsComparisonReference_ReturnsSingleValidationFailure ()
    {
      ...
      var validator = new NotEqualValidator (instanceToValidate);

      var validationFailures = validator.Validate (propertyValidatorContext).ToArray();

      Assert.That (validationFailures.Length, Is.EqualTo (1));
      Assert.That (validationFailures[0].ErrorMessage, Is.EqualTo ("Enter a value not equal to 'System.Object'."));
      Assert.That (validationFailures[0].LocalizedValidationMessage, Is.EqualTo ("Enter a value not equal to 'System.Object'."));
    }
    ...
    public void Validate_WithCustomComparer_UsesComparer ()
    {
      ...
      var validator = new NotEqualValidator ("comparisonValue", equalityComparerMock);

      var validationFailures = validator.Validate (propertyValidatorContext).ToArray();

      equalityComparerMock.VerifyAllExpectations();
      Assert.That (validationFailures.Length, Is.EqualTo (1));
      Assert.That (validationFailures[0].ErrorMessage, Is.EqualTo ("Enter a value not equal to 'comparisonValue'."));
    }
  }
}