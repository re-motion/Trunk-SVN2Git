...
namespace Remotion.Validation.UnitTests.Validators
{
  ...
  public class MaximumLengthValidatorTest : ValidatorTestBase
  {
    ...
    public void Validate_WithPropertyValueNull_ReturnsNoValidationFailures ()
    {
      ...
      var validator = new MaximumLengthValidator (0);
    ...
    public void Validate_WithPropertyValueIsStringWithMaxLength_ReturnsNoValidationFailures ()
    {
      ...
      var validator = new MaximumLengthValidator ("some string".Length);
    ...
    {
      var propertyValidatorContext = CreatePropertyValidatorContext ("some string");
      var validator = new MaximumLengthValidator ("some string".Length - 1);
      ...
      Assert.That (validationFailures[0].ErrorMessage, Is.EqualTo ("Enter between 0 and 10 characters."));
      Assert.That (validationFailures[0].LocalizedValidationMessage, Is.EqualTo ("Enter between 0 and 10 characters."));
    }

    [Test]
    public void Validate_WithPropertyValueIsStringWithLengthGreaterThanMaxLengthAndCustomValidationMessage_ReturnsSingleValidationFailureAndCustomValidationMessage ()
    {
      var propertyValidatorContext = CreatePropertyValidatorContext ("some string");
      var validator = new MaximumLengthValidator (2, new InvariantValidationMessage ("Custom validation message: '{MinLength}', '{MaxLength}'."));

      var validationFailures = validator.Validate (propertyValidatorContext).ToArray();

      Assert.That (validationFailures.Length, Is.EqualTo (1));
      Assert.That (validationFailures[0].ErrorMessage, Is.EqualTo ("Enter between 0 and 2 characters."));
      Assert.That (validationFailures[0].LocalizedValidationMessage, Is.EqualTo ("Custom validation message: '0', '2'."));
    }
  }
}