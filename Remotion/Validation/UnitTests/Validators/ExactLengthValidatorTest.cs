...
namespace Remotion.Validation.UnitTests.Validators
{
  ...
  public class ExactLengthValidatorTest : ValidatorTestBase
  {
    ...
    public void Validate_WithPropertyValueNoString_ReturnsNoValidationFailures ()
    {
      ...
      var validator = new ExactLengthValidator (0);
    ...
    public void Validate_WithStringAndStringLengthEqualsExactLength_ReturnsNoValidationFailures ()
    {
      ...
      var validator = new ExactLengthValidator ("some string".Length);
    ...
    public void Validate_WithStringAndStringLengthShorterThanExactLength_ReturnsSingleValidationFailure ()
    {
      ...
      var validator = new ExactLengthValidator ("12345".Length);
      ...
      Assert.That (validationFailures[0].ErrorMessage, Is.EqualTo ("Enter exactly 5 characters."));
      Assert.That (validationFailures[0].LocalizedValidationMessage, Is.EqualTo ("Enter exactly 5 characters."));
    }
    ...
    {
      var propertyValidatorContext = CreatePropertyValidatorContext ("1234");
      var validator = new ExactLengthValidator ("123".Length);

      var validationFailures = validator.Validate (propertyValidatorContext).ToArray();

      Assert.That (validationFailures.Length, Is.EqualTo (1));
      Assert.That (validationFailures[0].ErrorMessage, Is.EqualTo ("Enter exactly 3 characters."));
    }

    [Test]
    public void Validate_WithStringAndStringLengthGreaterThanExactLengthAndCustomValidationMessage_ReturnsSingleValidationFailureAndCustomValidationMessage ()
    {
      var propertyValidatorContext = CreatePropertyValidatorContext ("1234");
      var validator = new ExactLengthValidator ("123".Length, new InvariantValidationMessage ("Custom validation message: '{MaxLength}'."));
